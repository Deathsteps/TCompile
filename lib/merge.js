...
function process(filename){
	return new Promise(function (resolve, reject){
		...
		if(sys.path.extname(filename) == '') filename += '.js'
		var file = sys.path.join(CONFIG.path.src, filename);
		var destname = filename.replace('.js', '.merge.js');

		compileView(file)
			.then(function (view) {
				return sys.fs.readFileAsync(file, 'utf-8');
			}).then(function (code){
				...
				var tempFile = sys.path.join(CONFIG.path.src, destname);
