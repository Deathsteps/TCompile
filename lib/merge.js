		...
		sys.fs.readFileAsync(file, 'utf-8')
			.then(function (code){
				...
				sys.fs.writeFile(tempFile, code, function (err) {
					if (err) throw err;
					console.log('merge ok: ' + filename);
					resolve('merge.' + filename);
				});
			}).catch(function (err){
				reject('[merge] fail to read target file ' + filename);
			});
	});	
}
	...
	return new Promise(function (resolve, reject){
		...
		sys.fs.readFileAsync(file, 'utf-8')
			.then(function (code){
				...
				console.log('[merge] fail to read dependent file ' + file);
				reject(file);
			});
	});
}
	...
	return new Promise(function (resolve, reject){
		...
		transform(files)
			.then(function (code){
				...
				reject('[merge] fail to transform the dependent file ' + filename);
			});
