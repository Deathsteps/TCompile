...
function compressTarget (files) {
	files.forEach(function (filename) {
		var file = new File(sys.path.basename(file), '', sys.path.dirname(file));
		compress(file);
	});
}

function initConfig () {
	if(CONFIG.path.src && CONFIG.path.src.indexOf(CONFIG.path.project) != -1) return;
	["src", "min", "tools", "mods"].forEach(function (item, i) {
		CONFIG.path[item] = sys.path.join(CONFIG.path.project, item);
	});
	// deal with template helpers file path
	if(!CONFIG.template.helperPath)
		CONFIG.template.helperPath = sys.path.join(CONFIG.path.tools, 'helpers.js');
}
